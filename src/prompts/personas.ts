...
export const DEFAULT_ROLE_ID: RoleId = "co-van-hoc-duong";
...
export const PERSONAS: PersonaDefinition[] = [
  {
    ...
    description:
      "Ấm áp, trấn an nhẹ nhàng, dùng từ đơn giản và gần gũi.",
    prompt: `
...
`.trim()
  },
  {
    ...
    description:
      "Điềm tĩnh, có cấu trúc, giúp ổn định cảm xúc và suy nghĩ rõ hơn.",
    prompt: `
...
`.trim()
  },
  {
    ...
    description:
      "Tập trung vào bối cảnh sinh viên, học tập, quan hệ gia đình, bạn bè và hỗ trợ trong trường.",
    prompt: `
...
`.trim()
  },
  {
    ...
    description:
      "Phản hồi chậm hơn, phản chiếu cảm xúc, giúp người dùng thấy mình được lắng nghe.",
    prompt: `
...
export const PUBLIC_ROLES: PublicRole[] = PERSONAS.map(
  ({ id, name, description }) => ({
    id,
    name,
    description
...
export function getPersonaById(roleId?: string): PersonaDefinition {
