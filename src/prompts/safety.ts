export type RiskLevel = "safe" | "distress" | "high_risk";
...
export function composeSystemPrompt(input: {
  ...
  riskLevel: Exclude<RiskLevel, "high_risk">;
}): string {
...
export function createHelpNowReply(options?: {
  helpNowContactText?: string;
}): string {
  const helpNowContactText =
    options?.helpNowContactText?.trim() ||
