import type { SessionTurn } from "../types/session.js";
import {
...
function isFullDebugEnabled(): boolean {
...
function getGeminiApiKeys(): string[] {
  ...
  if (keys.length === 0) {
    throw new Error(
      "Missing Gemini API key. Set GEMINI_API_KEY_1 and GEMINI_API_KEY_2 in backend/.env"
    );
  }
...
function normalizeTurns(turns: SessionTurn[] | undefined): SessionTurn[] {
  if (!turns) return [];
...
function formatTurnsForPrompt(turns: SessionTurn[]): string {
  if (turns.length === 0) {
...
function formatStableFacts(stableFacts: string[] | undefined): string {
...
function buildChatUserPrompt(input: GenerateChatReplyInput): string {
...
function extractTextFromResponse(data: GeminiResponse): {
  text: string;
  finishReason?: string;
...
function looksIncomplete(text: string): boolean {
  ...
  const incompleteEnding =
    /(?:và|hay|hoặc|nhưng|vì|nên|rồi|để|khi|nếu|là|thì|còn|giờ|lúc này|bây giờ)$/i;
  if (incompleteEnding.test(cleaned)) return true;
...
async function generateModelText(input: {
  ...
  debugLabel?: string;
}): Promise<{ text: string; finishReason?: string; usage?: GeminiUsageMetadata }> {
...
): Promise<{ summary: string; stableFacts: string[] }> {
