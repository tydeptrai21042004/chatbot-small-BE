import { z } from "zod";
import { DEFAULT_ROLE_ID, getPersonaById, PERSONAS } from "../prompts/personas.js";
import { composeSystemPrompt, createHelpNowReply, postProcessAssistantReply } from "../prompts/safety.js";
import { generateChatReply } from "../services/gemini.js";
import { appendTurnToSession, buildMemoryContext, refreshSessionMemoryIfNeeded } from "../services/memory.js";
import { assessCrisisRisk } from "../utils/crisis.js";
import { getOrCreateSession, saveSession, saveSessionTurn } from "../utils/sessionStore.js";
const RoleIdSchema = z.enum(PERSONAS.map((item) => item.id));
...
const chatRoutes = async (app) => {
    app.post("/chat", async (request, reply) => {
        ...
        const { sessionId, message, roleId, customPersonaEnabled, customPersonaName, customPersonaPrompt } = parsed.data;
        ...
        try {
            ...
            const finalReply = postProcessAssistantReply(rawModelReply, crisis.riskLevel === "safe" ? "safe" : "distress");
            const userTurn = appendTurnToSession(session, "user", message);
            saveSessionTurn(session.sessionId, userTurn);
            ...
            const fallbackReply = crisis.riskLevel === "distress"
