...
function mergeUniqueFacts(facts) {
    const seen = new Set();
    const normalized = [];
...
function extractHeuristicFacts(turns) {
    ...
    if (userText.includes("không an toàn") ||
...
function buildFallbackSummary(existingSummary, olderTurns, existingStableFacts) {
    ...
    const summary = sanitizeText(summaryParts.join("\n\n") || "Đã có hội thoại trước đó cần được tham chiếu khi trả lời tiếp.", MAX_SUMMARY_LENGTH);
...
export function createSessionTurn(role, content) {
    return {
        role,
...
export function appendTurnToSession(session, role, content) {
...
export function buildMemoryContext(session) {
    return {
...
export async function refreshSessionMemoryIfNeeded(session) {
    ...
    try {
        ...
        session.rollingSummary = sanitizeText(nextMemory.summary, MAX_SUMMARY_LENGTH);
        ...
        const fallback = buildFallbackSummary(session.rollingSummary, olderTurns, session.stableFacts);
